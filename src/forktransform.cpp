...
void poseCallback(const geometry_msgs::PoseStamped::ConstPtr& msg){
    ...
    br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "ref_objectfork", "fork_base"));


    //ROS_INFO("Broadcasted transformation from pose sensor on fork object to tf");
}


int main(int argc, char** argv){
    ros::init(argc, argv, "forktransform");
    ros::NodeHandle n;

    ros::Subscriber sub = n.subscribe("/fork_pose", 1000, &poseCallback); 
