...
void poseCallback(const geometry_msgs::PoseStamped::ConstPtr& msg){
    ...
    br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "ref_objectbluecup", "bluecup_base"));


    //ROS_INFO("Broadcasted transformation from pose sensor on bluecup object to tf");
}


int main(int argc, char** argv){
    ros::init(argc, argv, "objectpose_transform");
    ros::NodeHandle n;

    ros::Subscriber sub = n.subscribe("/bluecup_pose", 1000, &poseCallback); 
